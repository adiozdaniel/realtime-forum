	...
	userComments: [
		{
			...
			content:
				"Great explanation! This helped me understand the concepts better.",
			likes: 5,
			timestamp: "3h ago",
		},
		{
			...
			content:
				"I've been using similar patterns in my projects. Very effective!",
			likes: 3,
			timestamp: "1d ago",
		},
	],
...
function updateTheme() {
	...
	elements.darkModeToggle.innerHTML = `<i data-lucide="${
...
async function handleImageUpload(e) {
	...
	if (!ALLOWED_TYPES.includes(file.type)) {
		...
		elements.imageUpload.value = ""; // Clear the input
		return;
	}
	...
	try {
		const user = await authService.uploadProfilePic(formData);
		...
		if (user.error) {
			...
			localStorage.setItem("profilePic", JSON.stringify(user.data.image));
			state.profilePic = user.data.image;
			elements.profileImage.src = user.data.image;
...
function updateStats() {
	...
	document.getElementById("commentsCount").textContent =
		state.userComments.length;
	document.getElementById("likesCount").textContent = state.posts.reduce(
...
function renderComments() {
	...
	commentsList.innerHTML = state.userComments
		.map(
			(comment) => `
