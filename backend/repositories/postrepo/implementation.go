...
func (r *PostRepository) UpdatePost(post *Post) error {
	query := `UPDATE posts SET post_title = ?, post_content = ?, post_image = ?, post_video = ?, post_category = ?, post_likes = ?, updated_at = ? WHERE post_id = ?`
	_, err := r.DB.Exec(query, post.PostTitle, post.PostContent, post.PostImage, post.PostVideo, post.PostCategory, post.PostLikes, post.UpdatedAt, post.PostID)
	return err
}
